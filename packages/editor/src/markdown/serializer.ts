...
// Raw, lossless serializer for a single textblock. Used by the live inline-mark
// detection round-trip in `live/inline-marks.ts`: it serializes the current
// textblock to markdown, re-parses it, and diffs the result to discover newly
// completed inline syntax. Output must round-trip cleanly through the parser,
// so it does NOT escape characters or expel whitespace.
export function serializeTextblockToRawMarkdown(node: ProseMirrorNode): string {
  ...
  const inline = serializeRawInline(node);
  if (node.type.name === "heading") return `${"#".repeat(node.attrs.level)} ${inline}`;
  return inline;
}

function serializeRawInline(node: ProseMirrorNode): string {
  let out = "";
  let active: Mark[] = [];
  ...
  node.forEach((child) => {
    ...
    for (let i = active.length - 1; i >= shared; i -= 1) out += closeRawMark(active[i]);
    for (let i = shared; i < marks.length; i += 1) out += openRawMark(marks[i]);
    active = marks;
  ...
  for (let i = active.length - 1; i >= 0; i -= 1) out += closeRawMark(active[i]);
  return out;
}

function openRawMark(mark: Mark): string {
...
function closeRawMark(mark: Mark): string {
  switch (mark.type.name) {
    case "strong":
      return "**";
    case "em":
    ...
    case "code":
      return "`";
    case "link":
      return `](${(mark.attrs.href as string).replace(/[()]/g, "\\$&")})`;
    default:
      return "";
  }
}
